...
# --- Configuration ---
# NUM_CANDIDATE_ACTIONS = 50 # Increased for better choice, adjust as needed
...
while running and step_count < MAX_STEPS:
    ...
    print(f"Predicting outcomes for {len(candidate_actions)} actions (batched)...")
    pred_start_time = time.time()
    list_of_predicted_states, list_of_actions_used = predict_multiple_next_states_batched( # Call the new function
        current_perception,
        candidate_actions,
        world_model,
        scaler
    )
    print(f"  Batch prediction took: {time.time() - pred_start_time:.4f}s")
